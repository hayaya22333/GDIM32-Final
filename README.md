# GDIM32-Final
## Check-In
### Group Devlog
While working on the final project, we encounter a large merge conflict. We initially tried to resolve the conflict, but this caused many of our files and prefabs to get deleted. In order to restore these files and prefabs, we used the version control techniques that were taught in week 6. We used "discard changes" to discard current changes that were made to the scene. These were changes that Unity automatically created upon entering the scene, and thus, we simply discarded them. We then used "revert changes in commit" to revert the changes made in the merge commit that caused the errors to occur. After performing the revert, we were able to restore the version without all the missing assets. Finally, we used "create branch from commit" to create a temporary branch from the commit that caused the errors. We then copied and pasted the dialogue and NPC related files from the temporary branch into the real branch. After this process, we were able to restore the final project repo and also add in the new code.

### Leo Ave
Put your individual check-in Devlog here.
### Jing Chen
Put your individual check-in Devlog here.
### Han Yang
Put your individual check-in Devlog here.


## Final Submission
### Group Devlog
Put your group Devlog here.


### Leo Ave
Put your individual final Devlog here.
### Jing Chen
Put your individual final Devlog here.
### Han Yang
Put your individual final Devlog here.

## Open-Source Assets
Cite any open-source assets here. Put them in a LIST, and use correctly formatted LINKS.
